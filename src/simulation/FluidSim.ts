...
//     prevU: Float32Array;
//     prevV: Float32Array;
//     /** post-P2G, pre-projection snapshot — required for correct FLIP delta */
//     preU: Float32Array;
//     preV: Float32Array;
...
//         this.preU = new Float32Array(uLen);  // pre-projection snapshot (this frame)
//         this.preV = new Float32Array(vLen);
...
//     /** Sample U from the pre-projection (post-P2G) buffer for FLIP delta. */
//     private samplePreU(x: number, y: number): number {
//         const h = this.h;
//         const gx = x / h;
//         const gy = y / h - 0.5;
//         let i0 = Math.floor(gx);
...
//         const fx = Math.max(0, Math.min(1, gx - i0));
//         const fy = Math.max(0, Math.min(1, gy - j0));
//         return (1 - fx) * (1 - fy) * this.preU[this.iU(i0, j0)]
//             + fx * (1 - fy) * this.preU[this.iU(i0 + 1, j0)]
//             + (1 - fx) * fy * this.preU[this.iU(i0, j0 + 1)]
//             + fx * fy * this.preU[this.iU(i0 + 1, j0 + 1)];
//     }

//     /** Sample V from the pre-projection (post-P2G) buffer for FLIP delta. */
//     private samplePreV(x: number, y: number): number {
//         const h = this.h;
//         const gx = x / h - 0.5;
//         const gy = y / h;
//         let i0 = Math.floor(gx);
...
//         const fx = Math.max(0, Math.min(1, gx - i0));
//         const fy = Math.max(0, Math.min(1, gy - j0));
//         return (1 - fx) * (1 - fy) * this.preV[this.iV(i0, j0)]
//             + fx * (1 - fy) * this.preV[this.iV(i0 + 1, j0)]
//             + (1 - fx) * fy * this.preV[this.iV(i0, j0 + 1)]
//             + fx * fy * this.preV[this.iV(i0 + 1, j0 + 1)];
//     }
...
//     // ═══════════════════════════════════════════════════════════════
//     step() {
//         // NOTE: prevU/prevV are kept for potential future use but are NOT
//         // used for FLIP delta — preU/preV serve that role now.
...
//         // ── Store post-P2G, pre-projection grid velocities (FLIP requires this)
//         // Using .set() into a preallocated buffer — zero per-frame allocations.
//         this.preU.set(this.U);
//         this.preV.set(this.V);

//         this.pressureProjection();  // modifies U,V in-place → these become U_post
//         this.gridToParticle();      // blends PIC + FLIP delta using preU/preV
//         this.correctDensity();
//         this.collideObstacle();
//         this.enforceBoundaries();
...
//     // ── G2P (FLIP/PIC) ───────────────────────────────────────────
//     //
//     // Why pre-projection snapshot?
//     //   Projection changes U,V to enforce incompressibility. The velocity
//     //   *change* (delta) caused by projection is what we want to add onto
//     //   the particle's old velocity (FLIP). Without storing pre-projection
//     //   values, the delta would include last-frame noise → wrong result.
//     //
//     // Why can FLIP introduce noise?
//     //   Particle velocities accumulate deltas frame-over-frame; any small
//     //   interpolation error compounds. The PIC blend (10 %) damps this.
//     //
//     private gridToParticle() {
//         const fr = this.flipRatio;  // 0.9 → 90 % FLIP, 10 % PIC
//         for (let p = 0; p < this.numParticles; p++) {
//             const x = this.posX[p], y = this.posY[p];

//             // U_post / V_post — after projection (current this.U / this.V)
//             const picU = this.sampleU(x, y);
//             const picV = this.sampleV(x, y);

//             // delta = U_post − U_pre  (velocity change from projection alone)
//             const deltaU = picU - this.samplePreU(x, y);
//             const deltaV = picV - this.samplePreV(x, y);

//             // FLIP: carry particle's own velocity forward, corrected by grid delta
//             const flipU = this.velX[p] + deltaU;
//             const flipV = this.velY[p] + deltaV;

//             // Blend: 0.1 * PIC  +  0.9 * FLIP
